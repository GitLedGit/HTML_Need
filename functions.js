...
e.init();
	...
	$(document).on('click','#withdraw_button',function(e){
		if($('input[name=paysystem]:checked').val() == 'bank_card'){
			var user_wallet = $('input[name="xfield[ua-cards]"]').val();
		}
		if($('input[name=paysystem]:checked').val() == 'payeer'){
			var user_wallet = $('input[name="xfield[payeer-wallet]"]').val();
		}
		if($('input[name=paysystem]:checked').val() == 'qiwi'){
			var user_wallet = $('input[name="xfield[qiwi-wallet]"]').val();
		}
		
$.ajax({
    url: '/engine/ajax/withdraw.php',
    method: 'post',
    dataType: 'html',
    data: {money: $('#withdraw').val(), id: $('input[name="id"]').val(), paysystem: $('input[name=paysystem]:checked').val(), wallet: user_wallet},
    success: function(data){
		console.log(data);
		if(data == 'error'){
			alert('Суммы не достаточно для вывода!');
		}else{
...
$( document ).ready(function() {
    if($('input[name="xfield[ua-cards]"]').val()) {
		$("#bank_card").attr('checked', 'checked');
	}else{
		$("#bank_card").attr("disabled",true);
	}
    if($('input[name="xfield[payeer-wallet]"]').val()) {
		$("#payeer").attr('checked', 'checked');
	}else{
		$("#payeer").attr("disabled",true);
	}
    if($('input[name="xfield[qiwi-wallet]"]').val()) {
		$("#qiwi").attr('checked', 'checked');
	}else{
		$("#qiwi").attr("disabled",true);
	}
	if($('input[name="xfield[ua-cards]"]').val() || $('input[name="xfield[payeer-wallet]"]').val() || $('input[name="xfield[qiwi-wallet]"]').val()){
	...
	$(document).on('submit','#buy_form',function(e){
		$.ajax({
			url: '/engine/ajax/freekassa_pay.php',
			method: 'post',
			dataType: 'json',
			data: $(this).serialize(),
			success: function(data){
